...
const haveChildren = node => {
  if (!mapTypenameToChildrenKeys[node.__typename]) return false;
  const childrenKeys = mapTypenameToChildrenKeys[node.__typename];
...
const getNodeTitle = node => {
  if (node.__typename === 'ElementFactory.tsx') {
    return `${node.__typename}-${node.component.type}`;
  }
...
export const toTreeModel = (domNode, model: any = {}) => {
  ...
  if (haveChildren(clonedNode)) {
    const childrenKeys = mapTypenameToChildrenKeys[clonedNode.__typename];
...
export const traverseDom = (domNode, callback, path = '') => {
  ...
  const childrenKeys = mapTypenameToChildrenKeys[domNode.__typename] || [];
  let children: any = [];
